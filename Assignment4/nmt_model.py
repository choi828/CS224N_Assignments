...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embded_cnn = nn.Conv1d(in_channels=embed_size, out_channels=embed_size, kernel_size=2, padding=1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        permuted_X = X.permute(1, 2, 0) # (b, e, src_len)
        X_cnn = self.post_embded_cnn(permuted_X)
        original_X = X_cnn.permute(2, 0, 1) #(src_len, b, e)
        pad_packed_sequence_X = pack_padded_sequence(original_X,  source_lengths)
        enc_hiddens, (last_hidden, last_cell)  = self.encoder(pad_packed_sequence_X)
        enc_hiddens = pad_packed_sequence(enc_hiddens) 
        enc_hiddens = enc_hiddens[0].permute(1,0,2)


        # last_hidden 텐서의 순방향과 역방향 은닉 상태를 연결
        init_decoder_hidden_cat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(init_decoder_hidden_cat)

        init_decoder_cell_cat = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(init_decoder_cell_cat)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        time_steps = torch.split(Y, 1, dim=0)
