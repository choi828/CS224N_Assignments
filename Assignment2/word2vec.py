...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec -- the gradient with respect to the center word vector
                     in shape (word vector length, )
                     (dJ / dv_c in the pdf handout) U(y_hat - y)
    ...
    # 중심 단어 벡터와 외부 단어 벡터 사이의 점수 계산
    scores = np.matmul(outsideVectors, np.transpose(centerWordVec)) # shape (u,), (v,)

    # 소프트맥스 함수를 사용하여 점수를 확률로 변환
    probs = softmax(scores)

    y_hat = probs[outsideWordIdx]
    ...
    # 그래디언트 계산
    gradCenterVec = np.matmul(probs - y, outsideVectors) # shape (w, u), (y, ) U의 컬럼은 y의 행과 같다. 따라서 전치 없이 내적 가능하다. 

    # 그래디언트 계산 Vc(y_hat -y).T
    gradOutsideVecs = np.outer(probs - y, centerWordVec) # shape (w, v), (y, ) V의 컬럼은 y의 행과 같다. 따라서 전치 없이 내적 가능하다. 
...
def negSamplingLossAndGradient(
    ...
    gradOutsideVecs = np.zeros(outsideVectors.shape) # 이거 없어도 동작하는지 확인하기
